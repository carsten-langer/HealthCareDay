...
object Metric {
  ...
  def metric(topics: Topics, workshops: Workshops, studentsSelectedTopics: StudentsSelectedTopics)(workshopAssignments: WorkshopAssignments): Metric = {
    ...
    val studentMetrics = studentAssignments.map { case (studentId, workshopIds) =>
      val assignedTopicIds = workshopIds.map(workshops).toList.map { case (topicId, _, _, _) => topicId } // .toList is redundant to business logic
      val (_, selectedTopics) = studentsSelectedTopics(studentId)
      val selectionPriorities = assignedTopicIds.map(selectedTopics.get).map(_.getOrElse(SelectionPriority(7)))
      val categories = assignedTopicIds.map(topics)
