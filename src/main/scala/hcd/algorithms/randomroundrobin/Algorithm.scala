  ...
  protected[randomroundrobin] def distributeSingleRound: DistributionAlgorithm =
    (topics: Topics, workshops: Workshops) =>
      (studentsSelectedTopics: StudentsSelectedTopics) => {
        ...
        distributePreOrdered(topics, baseOrderedWorkshops, baseOrderedStudentsWithOrderedSelections)
      }

  /** This algorithm's distribution function for one round with pre-ordered workshops and students. */
  private def distributePreOrdered(
                                    topics: Topics,
                                    orderedWorkshops: List[Workshop],
                                    orderedStudents: List[Student],
